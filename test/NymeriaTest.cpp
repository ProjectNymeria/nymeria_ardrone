...
/**
 * Central functionality of the NymeriaTest: trigger Nymeria
 *in order to actiate
...
void NymeriaTest::loop(ros::NodeHandle * n){
        ...
        while( !myfile.eof() && !loopExit )
        {
                ...
                //get the line and convert it into
                // stringstream for parsing
                std::getline(myfile, line);
                std::stringstream strs(line);

                // Read the integers using the operator >>
                // numbers are separated by the tabulation
                // (\t) and attributed to each variable
